...
async def private_receive_handler(c: Client, m: Message):
    ...
    try:
        ...
        api_key = Var.API
        shortened_link = shorten_url(stream_link, api_key)
        
        online_link = f"{Var.URL}{str(log_msg.id)}/{quote_plus(get_name(log_msg))}?hash={get_hash(log_msg)}"
        shortened_online_link = shorten_url(online_link, api_key)
        
        msg_text ="""<b>📚 𝗬𝗼𝘂𝗿 𝗟𝗶𝗻𝗸 𝗚𝗲𝗻𝗲𝗿𝗮𝘁𝗲𝗱 ⇛\n\n📕 ꜰɪʟᴇ ɴᴀᴍᴇ : {}\n\n📊 ꜰɪʟᴇ ꜱɪᴢᴇ :{}\n\n**𝗦𝗵𝗼𝗿𝘁𝗻𝗲𝗱 𝗟𝗶𝗻𝗸𝘀\n\n💻 WATCH : {}\n\n📥 Dᴏᴡɴʟᴏᴀᴅ  : {}\n\n⚠️ Nᴏᴛᴇ : 𝙇𝙞𝙣𝙠 𝙬𝙞𝙡𝙡 𝙣𝙤𝙩 𝙚𝙭𝙥𝙞𝙧𝙚 𝙪𝙣𝙩𝙞𝙡 𝙞 𝙙𝙚𝙡𝙚𝙩𝙚</b>"""

        await log_msg.reply_text(text=f"**RᴇQᴜᴇꜱᴛᴇᴅ ʙʏ :** [{m.from_user.first_name}](tg://user?id={m.from_user.id})\n**Uꜱᴇʀ ɪᴅ :** `{m.from_user.id}`\n**Stream ʟɪɴᴋ :** {stream_link}", disable_web_page_preview=True,  quote=True)
        await m.reply_text(
            text=msg_text.format(get_name(log_msg), humanbytes(get_media_file_size(m)), shortened_link, shortened_online_link),
            quote=True,
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup([
